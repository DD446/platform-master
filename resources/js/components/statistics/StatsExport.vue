...
<script>
    ...
    export default {
        ...
        methods: {
            onSubmit (evt) {
                evt.preventDefault();
                axios.post('/beta/statistiken/export', this.form)
                    .then(response => {
                        this.showAlert(response.data.message);
                        this.getExports();

                        Echo.private('spotify.stats.' + response.data.id)
                            .listen('\\App\\Events\\StatisticsExportStarted', (e) => {
                                let idx = this.exports.findIndex(x => x.id === response.data.id);
                            })
                            .listen('\\App\\Events\\StatisticsExportCreated', (e) => {
                                // @TODO
                                this.getExports();
                            });
                    })
...
/*                axios.get('/spotify/statistiken/first')
            ...
            getExports() {
                axios.get('/beta/statistiken/exports')
                    .then(response => {
            ...
            deleteExport(row) {
                ...
                if (confirm("Willst Du den Export wirklich löschen?")) {
                    ...
                    axios.delete('/beta/statistiken/exports/' + id)
                        .then((response) => {
                                this.showAlert(response.data.message);
                            },
                            (error) => {
        ...
        mounted() {
            eventHub.$on("stats-export-modal:show", () => {
                this.show();
            });
            this.getMinimumDate();
            this.getFiles();
