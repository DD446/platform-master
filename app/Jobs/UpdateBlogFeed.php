...
class UpdateBlogFeed implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $podcast = [
            $lfeed => [
                'feed_id'           => $feed,
                'name'              => $oFeed->rss['title'] ?? 'Podcast',
                'description'       => $oFeed->rss['description'] ?? 'Podcast',
            ]
        ];
    ...
    public function fail(\Exception $exception = null)
    {
        ...
        Log::error("User: {$this->username}. Updating blog feed. Job failed: " . $exception instanceof \Exception ? $exception->getTraceAsString() : null);
    }
